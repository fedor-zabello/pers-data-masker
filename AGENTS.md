...
- **`application.yaml` сейчас сломан**: `spring.threads.virtual.enabled:` без
  ...
  поставь `true`.
- **Masker работает справа налево** (`Masker.java:35`): спаны сортируются по
  `start` по убыванию, чтобы замена переменной длины не сдвигала ещё не
  обработанные позиции. Не меняй порядок.
